    ...
    for j in range (0, len(benchmarkfunc)):
        ...
        if((optimizer[i]==True) and (benchmarkfunc[j]==True)): # start experiment if an optimizer and an objective function is selected
            for k in range (0,NumOfRuns):
                ...
                avgconvergence+=x.convergence[-1];
                avgtime+=x.executionTime;
                ...
                if(Export==True):
                    with open(ExportToFile, 'a') as out:
                        ...
                        if k==NumOfRuns-1:
                            writer.writerow([x.objfname,'AverageTime',(avgtime/NumOfRuns),'','AverageConvergence',(avgconvergence/NumOfRuns)]);
                            writer.writerow(['']);
                    out.close();
